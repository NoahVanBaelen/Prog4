...
PlotStatus Plot(const char* label, const PlotConfig& conf) {
    ...
    const ImRect frame_bb(window->DC.CursorPos,ImVec2(window->DC.CursorPos.x + conf.frame_size.x, window->DC.CursorPos.y + conf.frame_size.y));
    const ImRect inner_bb(ImVec2(frame_bb.Min.x + style.FramePadding.x, frame_bb.Min.y + style.FramePadding.y), ImVec2(frame_bb.Max.x - style.FramePadding.x, frame_bb.Max.y - style.FramePadding.y));
    ...
    if (conf.values.count > 0) {
        ...
        float x_max = static_cast<float>(conf.values.offset + conf.values.count) - 1.f;
        if (conf.values.xs) {
        ...
        if (conf.grid_x.show) {
            ...
            case PlotConfig::Scale::Linear: {
                ...
                float inc = 1.f / cnt;
                for (int i = 0; i <= cnt; ++i) {
                    int x0 = static_cast<int>(ImLerp<float>(inner_bb.Min.x, inner_bb.Max.x, i * inc));
            ...
            case PlotConfig::Scale::Log10: {
                ...
                while (start < x_max) {
                    for (int i = 1; i < 10; ++i) {
                        ...
                        int x0 = static_cast<int>(ImLerp<float>(inner_bb.Min.x, inner_bb.Max.x, t));
                        window->DrawList->AddLine(
                            ImVec2(static_cast<float>(x0), static_cast<float>(y0)),
                            ImVec2(static_cast<float>(x0), static_cast<float>(y1)),
        ...
        if (conf.grid_y.show) {
            ...
            float inc = 1.f / cnt;
            for (int i = 0; i <= cnt; ++i) {
                int y0 = static_cast<int>(ImLerp<float>(inner_bb.Min.y, inner_bb.Max.y, i * inc));
                window->DrawList->AddLine(
                    ImVec2(static_cast<float>(x0), static_cast<float>(y0)),
