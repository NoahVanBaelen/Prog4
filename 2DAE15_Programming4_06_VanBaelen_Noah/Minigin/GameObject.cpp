...
{
	if (!CheckIfNewParentIsOurChild(parent.get()))//Check if the new parent is one of your children/grandchildren our if our new parent this gameobject is
	{
		if (m_parent != nullptr)
		{
...
{
	m_children.emplace_back(std::make_shared<GameObject>(*child));
}

void dae::GameObject::RemoveChild(dae::GameObject* child)
{
	m_children.erase(std::find(m_children.begin(), m_children.end(), std::make_shared<GameObject>(*child)));
