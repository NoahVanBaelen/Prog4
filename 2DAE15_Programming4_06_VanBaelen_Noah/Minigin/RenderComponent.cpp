...
void RenderComponent::Render() const
{
	if (m_pTexture != nullptr)
	{
		...
		{
			for (int i = 0; i < m_pDstRectVector.size(); i++)
			{
		...
		else
		{
			if (m_pSrcRect)
			{
				...
				dae::Renderer::GetInstance().RenderTexture(*m_pTexture, pos.x, pos.y, m_width, m_height, *m_pSrcRect.get());
			}
			else
			{
				const auto& pos{ GetOwner()->GetWorldPosition() };
				dae::Renderer::GetInstance().RenderTexture(*m_pTexture, pos.x, pos.y, m_width, m_height);
			}
		}*/
	}
}
...
//{
//	m_pSrcRectVector[index]->x = srcX;
//	m_pSrcRectVector[index]->y = srcY;
//}
//
